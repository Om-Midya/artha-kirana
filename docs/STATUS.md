...
## Next up (handed off — not started)

- **Data-layer restructure** — auto-compute sale price from inventory `sellPrice × qty` when the LLM gives qty but no amount; tighter entity linking / FKs / indices; analytics-friendly schema; resolve the "customer/shop profile" idea against `CLAUDE.md §17` (no user auth). Full brief + open questions for the next agent: **`docs/superpowers/specs/2026-06-14-data-layer-direction.md`**. Next agent should brainstorm it (open questions there) → spec → plan → build.
