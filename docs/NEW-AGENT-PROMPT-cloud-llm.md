...
- **Keys: ALREADY CONFIGURED ✅** — `keys.properties` (gitignored, at repo root) already holds `OPENROUTER_KEY` (a real OpenRouter key the user provided), `OPENROUTER_MODEL=anthropic/claude-haiku-4.5`, `OPENROUTER_VISION_MODEL=anthropic/claude-haiku-4.5`. Your job in task A is to make `app/build.gradle.kts` read those into BuildConfig fields (mirror the colleague's `build.gradle.kts`: `Properties().load(rootProject.file("keys.properties"))` → `buildConfigField`). DO NOT print, echo, or commit the key value; DO NOT hardcode it. `keys.properties` is in `.gitignore` — keep it that way. Provider decision is settled: **OpenRouter + Claude Haiku 4.5.** INTERNET permission already granted; OpenRouter is HTTPS (network_security_config loopback-cleartext-only is unaffected).
- **Branch: settled — stay on `feat/analytics-chat`** (15 commits, unmerged, atop merged `main`). Build the cloud pivot here (it's an orthogonal swap under the `chat()` chokepoint). Do NOT branch off / merge to main first.
...
Start by reading the four docs + the colleague's reference files, then **brainstorm the remaining open questions with the user** before writing any code. NOTE — three are already SETTLED: provider = **OpenRouter + Claude Haiku 4.5**; API key = **already in gitignored `keys.properties`**; branch = **stay on `feat/analytics-chat`**. Still open: cloud-timeout-before-fallback (~8–12s suggested), `response_format` json_object confirmation, OCR scope (cloud bill only), keep-our-`SaleEntry`-schema, and how much UI polish to adopt.
